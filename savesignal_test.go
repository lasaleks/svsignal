...
func TestMultiplyInsertValues(t *testing.T) {
	var wg sync.WaitGroup
	ctx := context.Background()
...
// Тестирование запроса данных с периодом в который попадают данные буффера на запись
func TestQueryDataFromCache(t *testing.T) {
	var wg sync.WaitGroup
	ctx := context.Background()
	...
	for _, tc := range tt {
		t.Run(tc.caseName, func(t *testing.T) {
			...
			if tc.resp_begin_row != nil {
				mock.ExpectQuery(fmt.Sprintf("SELECT (.+) WHERE signal_id=%d and id=", tc.signal_id)).WillReturnRows(tc.resp_begin_row)
			}
			...
			if tc.resp_end_row != nil {
				mock.ExpectQuery(fmt.Sprintf("SELECT (.+) WHERE signal_id=%d and id=", tc.signal_id)).WillReturnRows(tc.resp_end_row)
			}
